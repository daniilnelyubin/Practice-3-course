...
std::vector<std::string> *GenerateVariance(std::string iString) {
  ...
  if ((int) iString.find('?') >= 0) {
    ...
    std::vector<std::string> *retVec1 = GenerateVariance(copy1);
    std::vector<std::string> *retVec2 = GenerateVariance(copy2);
    retVec1->insert(retVec1->end(), retVec2->begin(), retVec2->end());
    delete retVec2;
    return retVec1;
  } else {
